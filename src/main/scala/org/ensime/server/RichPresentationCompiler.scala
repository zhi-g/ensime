    ...
    */
	...
	if (p.overlaps(treeP)) {
          t match {
            case Ident(_) => {
              val sym = t.symbol
	      println("Ident: " + List(sym.nameString,treeP.start, treeP.end).toString())
              if (sym.isMethod) {
		syms += SymbolDesignation(treeP.start, treeP.end, 'method)
              } else if (sym.isVariable) {
		syms += SymbolDesignation(treeP.start, treeP.end, 'var)
              } else if (sym.isValue) {
		syms += SymbolDesignation(treeP.start, treeP.end, 'val)
              }
            }
	      ...
	      syms += SymbolDesignation(treeP.start, treeP.end, 'method)
            ...
            case _ => {
              val sym = t.symbol
	      if(sym != null && treeP != null){
		println("Ignoring: " + List(sym.nameString,treeP.start,treeP.end).toString())
